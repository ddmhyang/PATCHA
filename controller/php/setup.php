...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['username']) && isset($_POST['password'])) {
        ...
        if (empty($admin_username) || empty($admin_password)) {
            ...
            try {
                // --- (A) DB 연결 및 테이블 생성 ---
                // (db_connect.php를 여기서 직접 실행하여 $pdo 생성)
                $dsn = "sqlite:" . $db_file;
                $options = [
    ...
    // --- 2. (최초 접속 시) DB 파일이 이미 있는지 확인 ---
    if (file_exists($db_file)) {
        // (보안) DB 파일이 이미 존재하면, 계정 생성을 막습니다.
        ...
        $setup_success = true; // (설치가 이미 되어있으므로 폼을 숨김)
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        .container { background: #fff; border: 1px solid #ccc; padding: 30px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
        .form-group { margin-bottom: 15px; }
        .form-group label { display: block; margin-bottom: 5px; font-weight: bold; }
        .form-group input { padding: 10px; width: 300px; border: 1px solid #ccc; border-radius: 4px; }
        ...
        .message-box { margin-top: 20px; padding: 15px; border-radius: 5px; background-color: #f0f0f0; }
    </style>
</head>
<body>
    <div class="container">
